...
void connection_callback(ur_rpc_connection_status_t status, void* user_data) {
    ...
    if (status == UR_RPC_CONN_CONNECTED) {
        ...
        // Subscribe to messages from Client A
        ur_rpc_subscribe_topic(g_client, "clients/client_a/messages");
        ur_rpc_subscribe_topic(g_client, "clients/client_a/rpc/+/+");
        printf("[%s] Subscribed to Client A topics\n", client_name);
    }
}
    ...
    if (strstr(topic, "/rpc/") != NULL) {
        ...
        if (json) {
            ...
            if (method && transaction_id) {
                ...
                if (strcmp(method->valuestring, "get_status") == 0) {
                    cJSON_AddStringToObject(result, "status", "online");
                    cJSON_AddStringToObject(result, "client_id", "client_b");
                    cJSON_AddStringToObject(result, "version", "1.0.0");
                    cJSON_AddNumberToObject(result, "uptime_seconds", 120);
                    cJSON_AddStringToObject(result, "message", "Client B is running normally");
                } else {
                    cJSON_AddStringToObject(result, "message", "Hello from Client B!");
                    cJSON_AddStringToObject(result, "processed_by", "client_b");
                    cJSON_AddStringToObject(result, "method_called", method->valuestring);
                }
                ...
                if (response_json) {
                    ...
                    snprintf(response_topic, sizeof(response_topic), 
                            "clients/client_b/rpc/response/%s", transaction_id->valuestring);
    ...
    if (strstr(topic, "/messages") != NULL) {
        ...
        if (json) {
            ...
            if (type && from && 
                ...
                strcmp(from->valuestring, "client_a") == 0) {
                
                // Send pong response
                cJSON* pong = cJSON_CreateObject();
                cJSON_AddStringToObject(pong, "from", "client_b");
                cJSON_AddStringToObject(pong, "type", "pong");
                cJSON_AddStringToObject(pong, "in_response_to", "client_a_ping");
                cJSON_AddNumberToObject(pong, "timestamp", (double)ur_rpc_get_timestamp_ms());
                
                char* pong_str = cJSON_Print(pong);
                if (pong_str) {
                    ur_rpc_publish_message(g_client, "clients/client_b/messages", pong_str, strlen(pong_str));
                    printf("[%s] Sent pong response\n", client_name);
                    free(pong_str);
                }
                cJSON_Delete(pong);
            }
...
int main(int argc, char* argv[]) {
    const char* config_file = "client_b_config.json";
    ...
    printf("Client B starting with config: %s\n", config_file);
    ...
    // Set callbacks
    ur_rpc_client_set_connection_callback(g_client, connection_callback, (void*)"Client B");
    ur_rpc_client_set_message_handler(g_client, message_handler, (void*)"Client B");
    
    // Connect to broker
    printf("Client B connecting to broker...\n");
    ...
    printf("Client B is ready for bidirectional messaging\n");
    printf("Waiting for messages from Client A...\n");
    ...
    while (running && ur_rpc_client_is_connected(g_client)) {
        ...
        if (message_counter % 75 == 0) {  // Every 7.5 seconds (100ms * 75)
            cJSON* status = cJSON_CreateObject();
            cJSON_AddStringToObject(status, "from", "client_b");
            cJSON_AddStringToObject(status, "type", "status_update");
            cJSON_AddStringToObject(status, "status", "running");
            cJSON_AddNumberToObject(status, "counter", message_counter / 75);
            cJSON_AddNumberToObject(status, "timestamp", (double)ur_rpc_get_timestamp_ms());
            
            char* status_str = cJSON_Print(status);
            if (status_str) {
                ur_rpc_publish_message(g_client, "clients/client_b/messages", status_str, strlen(status_str));
                printf("[Client B] Sent status update #%d\n", message_counter / 75);
                free(status_str);
            }
            cJSON_Delete(status);
        }
        
        // Send periodic RPC calls to Client A
        if (rpc_counter % 125 == 0 && rpc_counter > 0) {  // Every 12.5 seconds
            ur_rpc_request_t* request = ur_rpc_request_create();
            ur_rpc_request_set_method(request, "get_info", "client_a");
            ur_rpc_request_set_authority(request, UR_RPC_AUTHORITY_USER);
            ur_rpc_request_set_timeout(request, 5000);
            
            cJSON* params = cJSON_CreateObject();
            cJSON_AddStringToObject(params, "requester", "client_b");
            cJSON_AddNumberToObject(params, "request_id", rpc_counter / 125);
            cJSON_AddStringToObject(params, "query_type", "info");
            ur_rpc_request_set_params(request, params);
            
            printf("[Client B] Sending RPC request #%d to Client A\n", rpc_counter / 125);
            ur_rpc_call_async(g_client, request, response_handler, (void*)"Client B");
    ...
    printf("Client B shutdown complete\n");
    return 0;
}